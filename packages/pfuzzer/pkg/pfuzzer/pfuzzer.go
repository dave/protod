...
func gatherValidOperations(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, value protoreflect.Value, factory func() protoreflect.Value, exists bool) []opData {
...
func gatherValidOperationsIgnoreCollections(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, value protoreflect.Value, factory func() protoreflect.Value, exists bool) []opData {
	switch field.Kind() {
...
func gatherValidOperationsList(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, list protoreflect.List, factory func() protoreflect.Value, exists bool) []opData {
	var ops []opData
	...
	ops = append(ops, opData{exists: exists, op: &pdelta.Op{
		Type:     pdelta.Op_Set,
		Location: location,
		Value:    &pdelta.Op_Object{Object: randomDeltaList(location, set, field, factory)},
	}})
	...
	if list != nil {
		for i := 0; i < list.Len(); i++ {
			ops = append(ops, gatherValidOperationsIgnoreCollections(pdelta.CopyAndAppendIndex(location, int64(i)), set+1, field, list.Get(i), childFactory, true)...)
		}
	}

	return ops
}
func gatherValidOperationsMap(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, m protoreflect.Map, factory func() protoreflect.Value, exists bool) []opData {
	var ops []opData
	// DONE delete
	if exists {
		ops = append(ops, opData{exists: exists, op: &pdelta.Op{
	...
	ops = append(ops, opData{exists: exists, op: &pdelta.Op{
		Type:     pdelta.Op_Set,
		Location: location,
		Value:    &pdelta.Op_Object{Object: randomDeltaMap(location, set, field.MapKey(), field.MapValue(), factory)},
	}})
	...
	for _, key := range keys {
		ops = append(ops, gatherValidOperationsIgnoreCollections(pdelta.CopyAndAppend(location, newLocatorKey(deltaMapKey(key.Interface()))), set+1, field.MapValue(), m.Get(key), m.NewValue, true)...)
	}

	// DONE gather ops for new child ->
	newKey := deltaMapKey(getRandomKey(field.MapKey()))
	ops = append(ops, gatherValidOperationsIgnoreCollections(pdelta.CopyAndAppend(location, newLocatorKey(newKey)), set, field.MapValue(), protoreflect.Value{}, factory().Map().NewValue, false)...)
...
func gatherValidOperationsMessage(location []*pdelta.Locator, set int, descriptor protoreflect.MessageDescriptor, message protoreflect.Message, factory func() protoreflect.Value, exists bool) []opData {
	var ops []opData
	if exists {
		ops = append(ops, opData{exists: exists, op: &pdelta.Op{
			Type:     pdelta.Op_Delete,
	...
	ops = append(ops, opData{exists: exists, op: &pdelta.Op{
		Type:     pdelta.Op_Set,
		Location: location,
		Value:    &pdelta.Op_Message{Message: pdelta.MustMarshalAny(randomMessage(location, set, factory).Interface())},
	}})
	...
	for oneOfIndex := 0; oneOfIndex < descriptor.Oneofs().Len(); oneOfIndex++ {
		...
		for fieldIndex := 0; fieldIndex < oneOfDescriptor.Fields().Len(); fieldIndex++ {
			...
			oneOf.Fields = append(oneOf.Fields, &pdelta.Field{Name: string(fieldDescriptor.Name()), Number: int32(fieldDescriptor.Number())})
		}
		ops = append(ops, opData{exists: exists, op: &pdelta.Op{
			Type:     pdelta.Op_Delete,
	...
	for i := 0; i < descriptor.Fields().Len(); i++ {
		...
		childLocation := pdelta.CopyAndAppendField(parent, string(childField.Name()), int32(childField.Number()))
...
func randomProtoValue(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, factory func() protoreflect.Value) protoreflect.Value {
	switch {
	case field.IsList():
...
func randomOpValue(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, factory func() protoreflect.Value) interface{} {
	switch {
	case field.IsList():
		return &pdelta.Op_Object{Object: randomDeltaList(location, set, field, factory)}
	case field.IsMap():
		return &pdelta.Op_Object{Object: randomDeltaMap(location, set, field.MapKey(), field.MapValue(), factory)}
	}
	return randomOpValueIgnoreCollection(location, set, field, factory)
}
func randomObjectValue(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, factory func() protoreflect.Value) *pdelta.Object {
...
func randomProtoList(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, factory func() protoreflect.Value) protoreflect.List {
	list := factory().List()
	for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
		list.Append(randomProtoValueIgnoreCollection(pdelta.CopyAndAppendIndex(location, int64(i)), set, field, list.NewElement))
	}
	return list
}

func randomDeltaList(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, factory func() protoreflect.Value) *pdelta.Object {
	var obs []*pdelta.Object
	for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
		obs = append(obs, randomObjectValue(pdelta.CopyAndAppendIndex(location, int64(i)), set, field, factory().List().NewElement))
	}
	return &pdelta.Object{V: &pdelta.Object_List{List: &pdelta.List{List: obs}}}
}
func randomProtoMap(location []*pdelta.Locator, set int, keyField, valueField protoreflect.FieldDescriptor, factory func() protoreflect.Value) protoreflect.Map {
	m := factory().Map()
	for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
		k := getRandomKey(keyField)
		protoKey := protoMapKey(k)
		deltaKey := deltaMapKey(k)
		val := randomProtoValueIgnoreCollection(pdelta.CopyAndAppend(location, pdelta.NewLocatorKey(deltaKey)), set, valueField, m.NewValue)
		m.Set(protoKey, val)
	}
	return m
}
func randomDeltaMap(location []*pdelta.Locator, set int, keyField, valueField protoreflect.FieldDescriptor, factory func() protoreflect.Value) *pdelta.Object {
	switch keyField.Kind() {
	case protoreflect.BoolKind:
		m := map[bool]*pdelta.Object{}
		for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
			key := rand.Intn(1) == 0
			value := randomObjectValue(pdelta.CopyAndAppendKeyBool(location, key), set, valueField, factory().Map().NewValue)
			m[key] = value
		}
	...
	case protoreflect.Int32Kind:
		...
		for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
			...
			value := randomObjectValue(pdelta.CopyAndAppendKeyInt32(location, key), set, valueField, factory().Map().NewValue)
			m[key] = value
		}
	...
	case protoreflect.Int64Kind:
		...
		for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
			...
			value := randomObjectValue(pdelta.CopyAndAppendKeyInt64(location, key), set, valueField, factory().Map().NewValue)
			m[key] = value
		}
	...
	case protoreflect.Uint32Kind:
		...
		for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
			...
			value := randomObjectValue(pdelta.CopyAndAppendKeyUint32(location, key), set, valueField, factory().Map().NewValue)
			m[key] = value
		}
	...
	case protoreflect.Uint64Kind:
		...
		for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
			...
			value := randomObjectValue(pdelta.CopyAndAppendKeyUint64(location, key), set, valueField, factory().Map().NewValue)
			m[key] = value
		}
	...
	case protoreflect.StringKind:
		...
		for i := 0; i < rand.Intn(RANDOM_COLLECTION_MAX_ITEMS); i++ {
			key := randomString()
			value := randomObjectValue(pdelta.CopyAndAppendKeyString(location, key), set, valueField, factory().Map().NewValue)
			m[key] = value
		}
...
func randomMessage(location []*pdelta.Locator, set int, factory func() protoreflect.Value) protoreflect.Message {
	message := factory().Message()
	for i := 0; i < message.Descriptor().Fields().Len(); i++ {
		field := message.Descriptor().Fields().Get(i)
		fieldLocation := pdelta.CopyAndAppendField(location, string(field.Name()), int32(field.Number()))
...
func randomOpValueIgnoreCollection(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, factory func() protoreflect.Value) interface{} {
	switch field.Kind() {
	case protoreflect.EnumKind:
		return &pdelta.Op_Scalar{Scalar: &pdelta.Scalar{V: &pdelta.Scalar_Enum{Enum: int32(randomEnum(field))}}}
	case protoreflect.MessageKind:
		return &pdelta.Op_Message{Message: pdelta.MustMarshalAny(randomMessage(location, set, factory).Interface())}
	case protoreflect.BoolKind:
...
func randomProtoValueIgnoreCollection(location []*pdelta.Locator, set int, field protoreflect.FieldDescriptor, factory func() protoreflect.Value) protoreflect.Value {
	switch field.Kind() {
	case protoreflect.EnumKind:
