...
func (m *Main) scanner(protoRoot string) func(fpath string, info os.FileInfo, err error) error {
	return func(fpath string, info os.FileInfo, err error) error {
		...
		if hasAnnotatedType && goPkgOptionPath == "" {
			panic("go_package option not found")
		}
		if hasAnnotatedType && dartPkgOption == "" {
			panic("dart_package option not found")
		}
