...
func genDart(root string) {
	...
	sb.WriteString("import 'package:pdelta/pdelta/pdelta_transform.dart';\n")
	sb.WriteString("\n")
	...
	sb.WriteString("  }\n")
	sb.WriteString("}\n")
	if err := ioutil.WriteFile(filepath.Join(root, "pdelta_transform_generated.dart"), []byte(sb.String()), 0666); err != nil {
		panic(err)
	}
...
func genGo(root string) {
	...
	if err := f.Save(filepath.Join(root, "transform-generated.go")); err != nil {
		panic(err)
	}
}
