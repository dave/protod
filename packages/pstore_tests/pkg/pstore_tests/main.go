...
var PersonTypeName = string((&pdelta_tests.Person{}).ProtoReflect().Descriptor().FullName())
var CompanyTypeName = string((&pdelta_tests.Company{}).ProtoReflect().Descriptor().FullName())
...
func (a *App) PersonGetRequest(ctx context.Context, requestBytes []byte) (*Person_Get_Response, error) {
	...
	return &Person_Get_Response{State: state, Person: document.(*pdelta_tests.Person)}, nil
}
