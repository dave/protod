...
func (m *Main) writeGo(goRoot, goPkg string) error {
	...
	for _, pkg := range m.Packages {
		...
		//goPkgFromRelDir := path.Join(goPkg, pkg.RelativeDir)
		//if goPkgFromRelDir != pkg.GoPackagePath {
		//	return fmt.Errorf("go package path from root %q and relative dir %q does not match path from proto option %q", goPkg, pkg.RelativeDir, pkg.GoPackagePath)
		//}
		...
		if err := f.Save(filepath.Join(goRoot, pkg.RelativeDir, fmt.Sprintf("%s.op.go", pkg.Name))); err != nil {
	...
	*/
	...
	if isBase {
		if scalar, ok := locatable.(*ScalarData); ok && scalar.Name == "string" {
			/*
				func (b String_scalar) Edit(from, to string) *delta.Op {
