...
//func (transformer *Op) TransformOld(op *Op, priority bool) *Op {
...
//			switch TreeRelationship(transformer.Location, op.Location) {
//			case TREE_EQUAL:
//				_, isOpDelta := op.Value.(*Op_Delta)
//				if isOpDelta {
//					// Op is trying to modify the value that transformer has already overwriten. Delete op.
//					return nil
//				}
//				if priority {
...
//				switch TreeRelationship(transformer.Location, op.Location) {
//				case TREE_ANCESTOR:
//					// Op is inserting into a descendent of the value that transformer has already replaced.
//					// Delete op.
//					return nil
//				default:
//					return proto.Clone(op).(*Op)
...
//				shifter := insertShifter(transformerItemValue.Index, priority, op.Type == Op_Insert && relationship == TREE_EQUAL)
...
//				out := proto.Clone(op).(*Op)
//				out.Location[locationIndex].V.(*Locator_Index).Index = shifter(oldValue)
//
...
//					out.Value.(*Op_Index).Index = shifter(oldValue)
//				}
//				return out
//			default:
//				// operations are independent
...
//			case TREE_ANCESTOR, TREE_EQUAL:
//				// index transformation function
//				shifter := moveShifter(from.Index, to.Index, priority)
//				// update the location in op
//				locationIndex := len(transformer.Location) - 1
//				oldValue := op.Location[locationIndex].V.(*Locator_Index).Index
//				out := proto.Clone(op).(*Op)
...
//			}
//
//			shifter := deleteShifter(deleteIndex.Index)
//
//			// update the location in op
//			locationIndex := len(transformer.Location) - 1
